...
// src/html/symbol-blocks.ts
var LINE_ROW_RE = /<tr([^>]*)\bdata-line="(\d+)"([^>]*)>([\s\S]*?)<\/tr>/g;
var LINE_NUMBER_CELL_RE = /<td class="([^"]*\bhlts-line-number\b[^"]*)">([\s\S]*?)<\/td>/;
function decorateLineTableWithSymbolBlocks(lineTableHtml, symbols, options = {}) {
  ...
  for (const match of matches) {
    ...
    const foldMemberIds = collapsibleActiveBlocks.filter((block) => lineNumber > block.startLine).map((block) => block.id);
...
function normalizeSymbolBlocks(symbols) {
  return symbols.map((symbol, index) => {
    const contentRange = symbol.contentRange;
    const startLine = contentRange?.start?.line ?? symbol.startLine;
    const endLine = contentRange?.end?.line ?? symbol.endLine ?? startLine;
    if (typeof startLine !== "number" || typeof endLine !== "number" || !Number.isFinite(startLine) || !Number.isFinite(endLine)) {
      return null;
    }
    return {
      id: String(index + 1),
  ...
  }).filter((value) => value !== null).sort((a, b) => a.startLine - b.startLine || b.endLine - a.endLine);
}
